...
impl ParseOutput {
    ...
    pub fn is_hotkey(&self) -> bool {
        matches!(self, ParseOutput::Hotkey(_))
    }
    pub fn extract_hotkey(&self) -> &Hotkey {
        match self {
            ParseOutput::Hotkey(hotkey) => hotkey,
...
impl Line {
    ...
    pub fn mark_line(line: &str) -> LineType {
        if line.trim().is_empty() || line.trim().starts_with(COMMENT_SYMBOL) {
            ...
            LineType::Command
        } else if line.starts_with(IMPORT_STATEMENT) {
            LineType::Statement
        } else {
            LineType::Key
        }
...
pub fn load(path: &Path) -> Result<Vec<ParseOutput>, Error> {
    ...
    for config in Config::load_and_merge(configs)? {
        for item in parse_config(config)? {
